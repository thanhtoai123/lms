...
export const ADMIN_NAV: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Lớp Trial", href: "/lop-trial", perm: "class:read", phase: 3, desc: "Lớp học thử (buổi lẻ): xếp lead vào buổi thử, đổi lịch phải ghi lý do và báo GV, kết quả buổi thử cập nhật trạng thái lead." },
    ],
  },
  {
    key: "students",
    ...
    items: [
      { label: "Học viên", href: "/students", perm: "student:read", phase: 3, desc: "Danh sách học viên theo cơ sở; hồ sơ 5 khối (thông tin, PH 1–2, sức khoẻ, học tập, ghi chú); import Excel; vòng đời bảo lưu / nghỉ." },
      { label: "Tài khoản phụ huynh", href: "/students/tai-khoan", perm: "student:read", phase: 3, desc: "Tài khoản PH: chờ kích hoạt (OTP Zalo) / đang hoạt động / khoá; cấp mã kích hoạt tại quầy; gửi lại OTP." },
      { label: "Đăng ký học", href: "/enrollments", perm: "enrollment:read", phase: 3, desc: "Ghi danh 7 trạng thái kèm mốc thời gian; khoá tiên quyết chặn ghi danh; số buổi gói, buổi bắt đầu; liên kết đơn hàng." },
      { label: "Chuyển lớp / cơ sở", href: "/chuyen-lop", perm: "enrollment:update", phase: 3, desc: "Wizard chuyển lớp/cơ sở: giữ số buổi còn lại, ghi lý do, báo GV hai lớp và PH." },
      { label: "Sắp hết khoá", href: "/students/sap-het-khoa", perm: "enrollment:read", phase: 3, desc: "Học viên còn ≤ N buổi → việc tái tục cho tư vấn, nhắc PH." },
      { label: "Hoàn thành khoá & chứng chỉ", href: "/hoan-thanh-khoa", perm: "enrollment:read", phase: 3, desc: "Xác nhận hoàn thành, cấp chứng chỉ, gợi ý khoá tiếp theo." },
      { label: "Học bạ", href: "/hoc-ba", perm: "report_card:read", phase: 3, desc: "Học bạ kỳ theo mốc buổi 5 / buổi 12; GV viết, giáo vụ duyệt, gửi PH." },
      { label: "Học bạ năng lực", href: "/report-cards", perm: "report_card:read", phase: 3, desc: "Đánh giá năng lực theo bộ tiêu chí của từng khoá (Tiêu chí học bạ)." },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Lịch tổng", href: "/lich", perm: "session:read", phase: 3, desc: "Lịch tuần/tháng toàn cơ sở theo phòng và GV; lọc theo khoá, lớp." },
      { label: "Điểm danh", href: "/attendance", perm: "attendance:read", phase: 3, desc: "Tra cứu điểm danh theo lớp/ngày; sửa hồi tố phải có lý do và báo GV chính." },
      { label: "Ảnh lớp học", href: "/media", perm: "media:read", phase: 3, desc: "Ảnh/video buổi học, lọc theo lớp; chỉ hiển thị học viên có đồng ý đăng ảnh; link ký hạn." },
      { label: "Duyệt ảnh", href: "/duyet-media", perm: "media:update", phase: 3, desc: "Hàng đợi ảnh chờ duyệt trước khi gửi PH." },
      { label: "Học bù", href: "/hoc-bu", perm: "makeup:read", phase: 3, desc: "Yêu cầu học bù: chọn buổi bù ở lớp khác cùng bài, theo dõi đã bù / chờ bù." },
      { label: "Cơ sở", href: "/centers", perm: "class:read", phase: 3, desc: "Danh sách cơ sở, địa chỉ, khu vực, giờ hoạt động." },
      { label: "Phòng học", href: "/rooms", perm: "class:read", phase: 3, desc: "Phòng học theo cơ sở, sức chứa, thiết bị." },
    ],
  },
  {
    key: "lms",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Cảnh báo rủi ro", href: "/canh-bao-rui-ro", perm: "care:read", phase: 3, desc: "Nghỉ liên tiếp, chuyên cần thấp, chờ bù — sinh từ điểm danh." },
  ...
  },
  {
    ...
    label: "Tài chính",
    items: [
      { label: "Đơn hàng", href: "/orders", perm: "finance:read", phase: 4, desc: "Đơn học phí/sản phẩm, kế hoạch trả góp, QR thanh toán." },
      { label: "Thanh toán", href: "/payments", perm: "finance:read", phase: 4, desc: "Sale ghi nhận → Kế toán xác nhận." },
      { label: "Công nợ", href: "/cong-no", perm: "finance:read", phase: 4, desc: "Công nợ theo học viên / cơ sở." },
      { label: "Thiếu học phí", href: "/thieu-hoc-phi", perm: "finance:read", phase: 4, desc: "Ghi danh đang học nhưng chưa đủ học phí." },
      { label: "Nhập giao dịch cũ", href: "/nhap-giao-dich-cu", perm: "finance:create", phase: 4, desc: "Import giao dịch từ hệ cũ / sổ sách." },
      { label: "Biến động số dư", href: "/bien-dong-so-du", perm: "finance:read", phase: 4, desc: "Webhook SePay, đối khớp tự động với đơn." },
      { label: "Hoàn tiền", href: "/hoan-tien", perm: "finance:read", phase: 4, desc: "Hoàn theo số buổi chưa học, có duyệt." },
      { label: "Phương thức TT", href: "/payment-methods", perm: "finance:read", phase: 4, desc: "Tài khoản nhận tiền theo cơ sở." },
      { label: "Hoa hồng", href: "/crm/commission", perm: "finance:read", phase: 4, desc: "Hoa hồng sale / người giới thiệu." },
    ],
  },
  {
    key: "web",
  ...
  },
  {
    ...
    items: [
      { label: "Tài khoản", href: "/users", perm: "system:read", phase: 3, desc: "Tài khoản đăng nhập, vai trò theo cơ sở, vai trò chính, khoá/mở." },
      { label: "Nhóm người dùng", href: "/user-groups", perm: "system:read", phase: 4, desc: "Nhóm để gán quyền/nhận thông báo." },
      { label: "Vai trò & quyền", href: "/roles", perm: "system:read", phase: 3, desc: "Ma trận vai trò × quyền (đọc từ policy engine)." },
      { label: "Cây tổ chức", href: "/to-chuc", perm: "system:read", phase: 4, desc: "Khu vực → cơ sở → phòng ban." },
      { label: "Audit Log", href: "/audit-log", perm: "audit:read", phase: 3, desc: "Nhật ký thao tác bất biến, lọc theo cơ sở/module/người." },
  ...
  },
  {
    ...
    label: "Báo cáo",
    items: [
      { label: "Báo cáo Lead", href: "/bao-cao/lead", perm: "report:read", phase: 3, desc: "Phễu, tỉ lệ chuyển, theo sale/nguồn/cơ sở/tháng, rụng ở bậc nào." },
      { label: "Báo cáo trải nghiệm", href: "/bao-cao/trial", perm: "report:read", phase: 3, desc: "Học thử → đăng ký; lấp đầy lớp trial." },
      { label: "Báo cáo đào tạo", href: "/bao-cao/dao-tao", perm: "report:read", phase: 3, desc: "Chuyên cần theo lớp: buổi, đi học, vắng, chờ bù, đã bù." },
      { label: "Báo cáo trung tâm", href: "/bao-cao/trung-tam", perm: "report:read", phase: 4, desc: "Doanh thu theo tháng/cơ sở, công nợ." },
      { label: "Hiệu suất giáo viên", href: "/bao-cao/hieu-suat-gv", perm: "report:read", phase: 3, desc: "Buổi đã dạy, chuyên cần, điểm học bạ TB." },
