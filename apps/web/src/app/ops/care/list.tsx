      ...
      {items.map((t) => (
        <div key={t.id} className={`card p-4 flex flex-wrap items-center gap-3 ${t.overdue ? "border-red-200" : ""}`}>
          ...
          <div className="min-w-0 flex-1">
            <div className="font-medium">{t.studentName} <span className="text-xs text-ink-400">{t.studentCode}</span>{t.classId && <Link href={`/ops/classes/${t.classId}`} className="text-xs text-brand-700 ml-2">{t.className}</Link>}</div>
