...
export default async function TransfersPage({ searchParams }: { searchParams: Promise<{ month?: string; kind?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <LeadSubnav active="/ops/leads/transfers" />
      <div><h1 className="text-2xl font-bold">Sổ chuyển lead</h1><p className="text-sm text-ink-600">Bàn giao giữa sale và chuyển lead liên cơ sở theo tháng — ai chuyển, cho ai, lý do, kết quả hiện tại.</p></div>
            ...
            <tbody className="divide-y divide-black/5">
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="p-3"><Link href={`/ops/leads/${r.leadId}`} className="text-brand-700 font-medium">{r.parentName}</Link></td>
