import { hasPermission, PAYMENT_METHOD_KIND_VI, type Actor } from "@satarobo/core";
...
const FOR_VI: Record<string, string> = { course: "Khoá học", product: "Sản phẩm", exam: "Kỳ thi", other: "Khác" };
...
export default async function MethodsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Phương thức thanh toán" desc="Tài khoản / hình thức nhận tiền. Phương thức dùng chung hiện ở mọi cơ sở; phương thức riêng chỉ hiện ở đơn của cơ sở đó. Chuyển khoản có mã BIN + số tài khoản sẽ tự tạo mã QR VietQR trên đơn." />
            ...
            <tbody className="divide-y divide-black/5">
              {rows.map((m) => (
                <tr key={m.id} className={m.isActive ? "" : "text-ink-400"}>
                  ...
                  <td className="p-3 text-xs">{m.kind === "bank_transfer" ? <>{m.bankName} · BIN {m.bankBin}<div className="font-mono">{m.accountNo}</div><div>{m.accountName}</div></> : "—"}</td>
                  ...
                  <td className="p-3 text-xs">{m.allowFor.map((a) => FOR_VI[a] ?? a).join(", ")}</td>
