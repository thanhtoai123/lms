...
import { DAY_STATUS_SHORT, DAY_STATUS_VI, REQUEST_KIND_VI, REQUEST_STATUS_VI, type DayStatus } from "@satarobo/core";
import { useTRPC } from "@/lib/trpc/client";
import { DAY_CELL, DayChip, hm, units, wdOf } from "@/components/hr-ui";
        ...
        <table className="text-xs">
          <thead>
            <tr className="text-ink-400">
              ...
              <th className="p-2 text-right">Công</th><th className="p-2 text-right">Phép</th><th className="p-2 text-right">Lương</th><th className="p-2 text-right">Muộn</th>
            </tr>
          </thead>
              ...
              <tr key={r.id}>
                ...
                {r.days.map((c) => (
                  <td key={c.date} className="px-0.5 py-1 text-center">
                    <button
                      title={`${DAY_STATUS_VI[c.status as DayStatus]}${c.shiftCode ? ` · ca ${c.shiftCode}` : ""}${c.inMin != null || c.outMin != null ? ` · ${hm(c.inMin)}–${hm(c.outMin)}` : ""}${c.lateMin ? ` · muộn ${c.lateMin}′` : ""}`}
                    ...
                    >
                      {c.override ? "✎" : DAY_STATUS_SHORT[c.status as DayStatus]}
                ...
                <td className="p-2 text-right tabular-nums">{r.summary.lateCount ? `${r.summary.lateCount}/${r.summary.lateMin}′` : ""}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function DayPanel({ staffId, date, canUpdate, onClose }: { staffId: string; date: string; canUpdate: boolean; onClose: () => void }) {
  ...
  const ov = useMutation(trpc.hr.overrideDay.mutationOptions({ onSuccess: () => { setErr(null); setReason(""); q.refetch(); router.refresh(); }, onError: (e) => setErr(e.message) }));
  const d = q.data;
  return (
      ...
      {d && (
        <>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span>Ca: {d.cell.shift ? `${d.cell.shift.name} ${d.cell.shift.startTime}–${d.cell.shift.endTime}` : "không có"}</span>
            <span>Vào/ra: <b className="tabular-nums">{hm(d.cell.inMin)} – {hm(d.cell.outMin)}</b></span>
          ...
          {d.cell.override && <div className="rounded-lg bg-indigo-50 p-2 text-xs">Đã chỉnh: {units(d.cell.override.units)} công ({d.cell.override.label}) — {d.cell.override.reason}</div>}
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              ...
              {d.punches.length === 0 ? <div className="text-xs text-ink-400">Không có</div> : d.punches.map((p) => (
                <div key={p.id} className="text-xs">{p.kind === "in" ? "Vào" : "Ra"} {new Date(p.at).toLocaleTimeString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh", hour: "2-digit", minute: "2-digit" })} · {p.source === "gps" ? `GPS${p.distanceM != null ? ` ${p.distanceM}m` : ""}` : p.source === "request" ? "từ đơn" : p.source}{p.note ? ` · ${p.note}` : ""}</div>
              ))}
            </div>
            <div>
          ...
          {canUpdate && d.canOverride && (
            <div className="flex flex-wrap items-end gap-2 border-t border-black/5 pt-2">
              ...
              <button className="btn-primary" disabled={reason.trim().length < 5 || ov.isPending} onClick={() => ov.mutate({ staffId, date, units: Number(unitsV), label, reason: reason.trim() })}>Chỉnh công</button>
              {d.cell.override && <button className="btn-ghost" disabled={reason.trim().length < 5 || ov.isPending} onClick={() => ov.mutate({ staffId, date, units: null, reason: reason.trim() })}>Bỏ chỉnh</button>}
            </div>
          )}
...
export function PeriodActions({ centerId, period, status, blockers, warnings, canLock, canUnlock }: { centerId: string; period: string; status: string; blockers: string[]; warnings: string[]; canLock: boolean; canUnlock: boolean }) {
  const trpc = useTRPC();
  const router = useRouter();
  const [reason, setReason] = useState("");
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs">
      {status === "open" && canLock && (
        <>
          <button className="btn-primary !py-1 text-xs" disabled={blockers.length > 0 || lock.isPending} onClick={() => lock.mutate({ centerId, period })}>Khoá kỳ công</button>
          {blockers.length > 0 && <span className="text-amber-700">{blockers.join(" · ")}</span>}
          {blockers.length === 0 && warnings.length > 0 && <span className="text-amber-700">{warnings.join(" · ")}</span>}
        </>
      )}
