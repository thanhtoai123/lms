    ...
    <section className="card space-y-3 p-4">
      ...
      {ws.actions.length > 0 ? (
        <div className="space-y-2">
          <input className="input" maxLength={500} placeholder="Lý do / ghi chú (bắt buộc khi trả về, huỷ; khi bắt đầu lớp chưa đủ sĩ số tối thiểu)" value={reason} onChange={(e) => setReason(e.target.value)} />
...
export function SchedulePanel({ classId, ws }: { classId: string; ws: WS }) {
  ...
  const saveDraft = useMutation(trpc.academics.classes.saveDraftSchedule.mutationOptions({ onSuccess: () => { m.ok("Đã lưu kế hoạch lịch."); setOpen(false); router.refresh(); }, onError: m.err }));
    ...
    <section className="card space-y-3 p-4">
          ...
          <tbody className="divide-y divide-black/5">
            {ws.phases.map((ph) => (
              <tr key={ph.id} className={ph.past ? "text-ink-400" : ""}>
                ...
                <td className="p-2 text-xs">{fmt(ph.effectiveFrom)} → {ph.effectiveTo ? fmt(ph.effectiveTo) : "hết khoá"}{ph.changeReason && <div className="text-amber-800">{ph.changeReason}</div>}</td>
      ...
      {open && ws.planning && (
        <div className="space-y-2 rounded-xl border border-black/10 p-3">
          <div className="label">Ca học trong tuần (áp dụng từ ngày khai giảng)</div>
          <SlotEditor rows={rows} onChange={setRows} rooms={rooms} teachers={teachers} />
          <div className="flex gap-2">
            <button className="btn-primary" disabled={saveDraft.isPending} onClick={() => saveDraft.mutate({ classId, slots: toSlotInput(rows) })}>Lưu lịch</button>
