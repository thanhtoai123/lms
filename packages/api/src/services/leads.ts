...
import {
  leadTransition, computeSla, normalizeVnPhone, maskPhone, OPEN_LEAD_STATUSES, visibleCenterIds, authorize, CONSENT_TEXT_VERSION, normalizeRefCode,
...
export async function leadInbox(ctx: ProtectedContext, input: { scope: "mine" | "center" | "all"; status?: LeadStatus; centerId?: string; q?: string; limit?: number }) {
  ...
  const items = rows
    .map(({ phoneNormalized, ...r }) => ({
      ...
      sla: computeSla(r.status, r.lastTouchAt.toISOString(), now, policy.sla),
