...
export const leadsRouter = router({
  inbox: protectedProcedure
    .input(z.object({ scope: z.enum(["mine", "center", "all"]).default("all"), status: z.enum(LEAD_STATUSES).optional(), centerId: uuid.optional(), q: z.string().max(100).optional(), limit: z.number().int().min(1).max(500).optional() }).default({ scope: "all" }))
