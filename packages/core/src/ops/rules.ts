...
const DEFS: { key: string; level: EnvLevel | ((prod: boolean) => EnvLevel); group: string; note: string; minLen?: number }[] = [
  ...
  { key: "SUPABASE_SERVICE_ROLE_KEY", level: (p) => (p ? "required" : "optional"), group: "Đăng nhập", note: "Chỉ dùng phía máy chủ: gửi lời mời / đặt lại mật khẩu, khoá tài khoản đăng nhập" },
  { key: "NEXT_PUBLIC_APP_URL", level: (p) => (p ? "required" : "optional"), group: "Đăng nhập", note: "Địa chỉ công khai của hệ thống (liên kết trong email)" },
  { key: "REQUIRE_MFA_ROLES", level: "optional", group: "Đăng nhập", note: "Vai trò bắt buộc xác thực 2 lớp, cách nhau dấu phẩy (mặc định SUPER_ADMIN)" },
